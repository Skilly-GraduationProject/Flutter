...
class ProviderServiceViewBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      ...
      onRefresh: () async {
        context.read<ProviderServiceCubit>().getProviderService(serviceId);
      },
      child: BlocConsumer<ProviderServiceCubit, ProviderServiceState>(
        ...
        builder: (context, state) {
          if (state.getServiceState == CubitState.failure) {
            return const Center(child: CustomErrorWidget());
          } else {
            ProviderService? service = state.service;
            return Skeletonizer(
              enabled: state.getServiceState == CubitState.loading,
              ignorePointers: state.getServiceState == CubitState.loading,
              child: Padding(
