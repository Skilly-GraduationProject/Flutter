...
class InProgressServiceViewBody extends StatelessWidget {
  const InProgressServiceViewBody({super.key, required this.service});
  final RequestService service;
  @override
  Widget build(BuildContext context) {
    return BlocListener<ProviderServiceCubit, ProviderServiceState>(
        listener: (context, state) {
          if (state.completeServiceState == CubitState.loading) {
            showCustomLoading(context);
          } else if (state.completeServiceState == CubitState.success) {
            GoRouter.of(context).pop(context);
            GoRouter.of(context).pop(context);
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text("تم تنفيذ الخدمه بنجاح")),
            );
          } else if (state.completeServiceState == CubitState.failure) {
            GoRouter.of(context).pop(context);
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text("حدث خطأ أثناء التنفيذ")),
            );
          }
        },
        child: Padding(
          ...
          child: CustomScrollView(slivers: [
            ...
            // ReviewsSection(reviews: service?.),
            const SliverGap(20),
