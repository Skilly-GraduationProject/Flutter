import 'package:dio/dio.dart';
import 'package:grad_project/core/helper/shared_prefrences.dart';

class ApiService {
  final Dio dio;

  ApiService({required this.dio});
  // static const String token =
  //     "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..9tFNMpiTbKB3HXxwxXZzmftcXzYvBsf2G4IGhCbgOdQ";
  Future<Options> _getOptions({bool? isMultipart}) async {
    return Options(headers: {
      'Accept': 'application/json',
      'Authorization': 'Bearer ${await loadToken()}',
      if (isMultipart == true) 'Content-Type': 'multipart/form-data',
    });
  }

  Future<Response> post(String endPoint, dynamic data,
      {String? token, bool? isMultipart}) async {
    var response = await dio.post(endPoint,
        data: data, options: await _getOptions(isMultipart: isMultipart));
    return response;
  }

  Future<Response> get(String endPoint,
      {String? token, Map<String, dynamic>? queryParameters}) async {
    var response = await dio.get(endPoint,
        queryParameters: queryParameters, options: await _getOptions());
    print(response.data);
    return response;
  }

  Future<Response> delete(String endPoint, Map<String, dynamic>? data,
      {String? token, Map<String, dynamic>? params}) async {
    var response = await dio.delete(endPoint,
        data: data, queryParameters: params, options: await _getOptions());
    return response;
  }

  Future<Response> put(
      {required String endPoint,
      dynamic data,
      String? token,
      Map<String, dynamic>? params}) async {
    var response = await dio.put(endPoint,
        data: data, queryParameters: params, options: await _getOptions());
    return response;
  }
}
